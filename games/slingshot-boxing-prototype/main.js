import {
  GESTURE, checkGuardTrigger, classifyRelease, isDodgeCorrect, computePunch,
...
function throwPunch(side, power) {
  ...
  addEffect("playerCharge", side, 0); // clear any lingering charge visual immediately
}
...
function onPointerUp(e) {
  ...
  if (!guarding && game.phase === "playing") {
...
function update(dt) {
  ...
  if (guarding && game.enemy.phase === "telegraph") {
    game.defendedThisTelegraph = true;
  }
  ...
  if (event) {
    ...
    if (event.type === "hit") {
      ...
      showFlash(guarding ? "GUARD!" : "AVOID!", 400);
    }
  }
...
function drawPlayerGloves(w, h) {
  ...
  for (const side of ["left", "right"]) {
    ...
    const isCharging = touch.active && !guarding && touch.side === side &&
      sideForX(touch.startX) === side;
    if (isCharging) {
      ...
      color = `rgb(${Math.round(255)}, ${Math.round(209 - power * 130)}, ${Math.round(102 - power * 90)})`;
    }
    if (game.playerPunch && game.playerPunch.side === side) {
      const t = 1 - Math.max(0, game.playerPunch.timer) / game.playerPunch.totalMs;
      y = gloveY - t * scale * 1.6;
      r = scale * 0.55;
      color = "#ffdd55";
    }
    if (guarding) {
      x = w / 2 + (side === "left" ? -scale * 0.35 : scale * 0.35);
      y = h * 0.72;
      color = "#7ab8ff";
    }
